...
;(async function () {
    ...
    if (
        ...
        throw Error(
            'CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET, CLOUDINARY_CLOUD_NAME env vars are required'
        )
    }
    ...
    const injectRemoteImages = async (code) => {
        ...
        const html = renderToString(
            React.createElement(getMDXComponent(temp_code))
        )
    ...
    const cloudinaryUpload = async (props) => {
        ...
        msgs.push(
            `Looking at image: ${props.src}, ${
                exists ? 'already exists' : 'processing'
            }`
        )
        if (!exists) {
            msgs.push(
                'Uploading image: ' + props.src + ' with ' + props.transforms
            )
            try {
                await cloudinary.v2.uploader.upload(props.src, {
                    public_id: props.public_id,
                    // overwrite: false,
                    eager: props.transforms.length
                        ? props.transforms.join(',')
                        : undefined,
            ...
            props.transforms.length &&
            !exists.derived.find(
                (d) => d.transformation === props.transforms.join(',')
            )
        ) {
            msgs.push(
                'Image already exists, adding new transformations ' +
                    props.public_id +
                    ' with ' +
                    props.transforms
            )
            try {
                await cloudinary.v2.uploader.explicit(props.src, {
                    type: 'upload',
                    public_id: props.public_id,
                    eager: props.transforms.join(','),
                })
                msgs.push(
                    `Succesfully Added Transformation for ${props.public_id} - ${props.transforms}`
                )
            } catch (err) {
                console.error({
                    'ADD TRANSFORM FAILED': props.public_id,
                    ...err,
                })
            }
        } else {
            msgs.push(
                'Image already exists, no new transformations added. ' +
                    props.public_id +
                    ' with ' +
                    props.transforms
            )
    ...
    const imageSourceReplace = (src) => {
        ...
        // console.error("IMG src replace",  src)
        ...
        const existingResourceUrl = `${start}${
        ...
        const remoteSource =
            ...
            `https://res.cloudinary.com/${
                ...
                transforms.length ? `${transforms.join(',')}/` : ''
            }${public_id}.${src.substring(imageIndex - 1).split('.')[1]}`
    ...
    try {
        for await (let mdxPath of mdxPaths) {
            ...
            if (frontmatter['og:image']) {
                const props = imageSourceReplace(
                    `h_630,w_1200,c_scale,f_png/c_fit,w_1100,b_rgb:000a,co_white,${
                        !frontmatter?.['image:disable_text']
                            ? `l_text:Arial_90_bold_center:${
                                  frontmatter?.['og:title'] ||
                                  frontmatter?.['title'] ||
                                  ''
                              }`
                            : ''
                    },${frontmatter?.['image:transforms'] ?? ''}/${
